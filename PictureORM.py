...
class PictureORMHelper(object):
    ...
    def query_all_with_Picture_name_password(self, users):
        ...
        userList=self.session.query(Picture).filter_by(title=users.title ).all()
        return userList

    def query_all_Picture(self):
        userList = self.session.query(Picture).all()
        print len(userList)
        return userList
